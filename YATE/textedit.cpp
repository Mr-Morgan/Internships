...
void TextEdit::runAction()
{
    scriptRun(static_cast<QAction*>(sender())->text());
}//void runAction();
...
void TextEdit::scriptRun(QString name)
{
    ...
    if (sBox.getScriptLanguage(name) == ScriptsBox::SL_Python) {
        if (sBox.getInputMode(name) == ScriptsBox::IM_SaveSelection) {
            buf = textCursor().selectedText();
            insertPlainTextSMode(QtConcurrent::run(worker, &ScriptsWorker::pyWork, sBox.getScript(name), buf));
        } else {
            if (sBox.getInputMode(name) == ScriptsBox::IM_Buffer) buf = toPlainText();
            insertPlainText(QtConcurrent::run(worker, &ScriptsWorker::pyWork, sBox.getScript(name), buf));
        }//if (sBox.getInputMode(name) == ScriptsBox::IM_SaveSelection)
    } else return;
...
void TextEdit::updateActions()
{
    ...
    actions.clear();
    for (auto i : sBox.getNames()) {
        QAction *act = new QAction (i, this);
        act->setShortcut(sBox.getKeySequence(i));
        connect(act, SIGNAL(triggered()), this, SLOT(runAction()));
